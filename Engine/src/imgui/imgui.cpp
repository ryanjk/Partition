...
float ImGui::DeltaDragBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_speed, float v_min, float v_max, int decimal_precision, float power)
{
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
...
float ImGui::DeltaDragFloat(const char* label, float* v, float v_speed, float v_min, float v_max, const char* display_format, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	ItemSize(total_bb, style.FramePadding.y);
	const float return_delta = DeltaDragBehavior(frame_bb, id, v, v_speed, v_min, v_max, decimal_precision, power);
